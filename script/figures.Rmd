...
pS3c <- ggplot(moi %>% filter(pf_pcr_infection_status == "positive", !is.na(moi)), mapping = aes(x = moi, fill = case_type, color = case_type)) +
...
ggsave(plot = figS3, "figures/figS3.png", width = 10, height = 7)
...
ggsave(plot = hap_dist, "figures/figS4.png", height = 5, width = 9)
...
figS5 <- ggarrange(NULL,
                           ggarrange(pS5a +
                             guides(fill = guide_legend(ncol = 1)) +
                             theme(legend.position = "right") +
                             labs(fill = "Site", title = ""),
                           ggarrange(ggarrange(pS5b,
                                              pS5c +
                                                labs(y = ""),
                                              common.legend = TRUE,
                                              legend = "right",
...
ggsave(plot = figS5, "figures/figS5.png", width = 12, height = 6)
...
ggsave(plot = ridge_csp, "figures/figS6.png", height = 12, width = 12)
```
...
ggsave(plot = ridge_ama, "figures/figS7.png", height = 12, width = 12)
```
...
figS8 <- ggplot(rbind(csp_imported %>% mutate(haplotype = paste0("csp ", haplotype), gene = "csp"), ama_imported %>% mutate(haplotype = paste0("ama1 ", haplotype), gene = "ama1")), aes(x = ymd(date_final), fill = reported_travel)) +
  geom_dotplot(binwidth = 14, dotsize = 1.5) +
  scale_y_continuous(NULL, breaks = NULL) +
  scale_x_date(date_labels = "%b %Y", limits = c(ymd("2018-08-01", "2019-12-01")), breaks = c(ymd("2018-09-01"), ymd("2019-10-01"))) +
  scale_fill_manual(values = c("#CAE8F7", "#6C61E5")) +
...
ggsave(plot = figS8, "figures/figS8.png", height = 10, width = 7)
...
figS9 <- ggplot(weights, aes(x = weight.hh.dist, fill = factor(network, levels = c("PCR-positive network", "Share at least 1 csp haplotype", "Share at least 1 ama1 haplotype")))) +
...
ggsave(plot = figS9, "figures/figS9.png", height = 3, width = 7)
