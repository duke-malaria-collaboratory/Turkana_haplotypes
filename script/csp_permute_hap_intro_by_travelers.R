...
csp_merged$date_final <- ymd(csp_merged$date_final)

####----defining function that outputs introductions score----####

intro.score.hap <- function(csp_merged){
  hapdate <- csp_merged %>%
  ...
  hapdate <- hapdate %>%
    filter(date_final > min(date_final))
  haplist <- unique(csp_merged$haplotype)
  introlist <- ""
...
embatalk.score.hap <- intro.score.hap(csp_merged)

saveRDS(embatalk.score.hap, "data/generated_tables/csp.embatalk.score.hap.RDS")

####----permuting traveler assignments----####


perm.test.hap <- function(csp_merged) {
  temp <- csp_merged %>% group_by(study_id) %>% summarise(reported_travel = first(reported_travel))
  temp$reported_travel <- sample(temp$reported_travel)
  csp_temp <- merge(csp_merged %>% select(-reported_travel), temp)
  df <- intro.score.hap(csp_temp)
  return(df)
}
...
set.seed(1)
many.perm.hap <- data.frame(t(replicate(1000, perm.test.hap(csp_merged), simplify = TRUE)))
percentile.hap <- ecdf(unlist(many.perm.hap$score))
percentile.hap(embatalk.score.hap$score)

saveRDS(many.perm.hap, "data/generated_tables/many.perm.hap_csp.RDS")
...
ggsave("figures/csp_imports_permutation.png")
